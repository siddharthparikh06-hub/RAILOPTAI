...
export default function Copilot() {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 h-[550px] flex flex-col justify-between shadow-2xl">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto space-y-4 my-3 pr-2">
        {messages.map((m, idx) => (
          ...
          >
            {m.sender === 'copilot' && (
              <div className="w-7 h-7 rounded-lg bg-blue-600 flex items-center justify-center text-white shrink-0 font-bold text-xs shadow-md">
                RO
              </div>
            )}
        ...
        <div className="flex flex-wrap gap-2 text-[10px] font-mono">
          {samplePrompts.map((prompt, idx) => (
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-slate-950 border border-slate-800 hover:border-blue-500 text-slate-300 transition-colors"
            >
              {prompt}
            </button>
          ))}
